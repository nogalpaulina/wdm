...
function patientFile_loadScreen(data) {
  ...
  RenderUtil.render('component/patient_file_screen', params, function(screen) { 
  ...
  function addNewPatientFile(patientFile, $modal) {
    var sessionData = { sessionId: app_client.sessionId}
  ...
  function deletePatientFile(fileId, section) {
    var done = function() {
    ...
    var jsonData = JSON.stringify({ sessionId: app_client.sessionId, id:fileId});
  ...
  function setupPatientFileUpload($modal) {
    ...
    uploader = new qq.FileUploader({
     ...
     action: 'patientFile/uploadFile?sessionId=' + app_client.sessionId,
     debug: true,
     multiple:false,
  ...
  function initPatientFile() {
    ...
    function loadPatientFiles() {
      ...
      $.post("patientFile/getFiles", {data:jsonData}, function(data) {
        parsedData = $.parseJSON(data);
        PatientFiles.clear();
        _.each(parsedData.files, function(file){
      ...
      if (!_.isNull($selectedRow)) {
        ...
        if (currentClientIsProvider() || currentClientIsPatientAndOwner(file)) {
          var callback = function() {
            deletePatientFile(id, $currentTab.data('folder'));
          }  
